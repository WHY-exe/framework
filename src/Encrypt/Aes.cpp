...
std::tuple<gsl::span<uint8_t>, misc::ErrorCode> AES::Encrypt(gsl::span<const uint8_t> input, gsl::span<uint8_t> output) {
	if (ctx_.Ctx() == nullptr) {
		return { {}, sslerr::MakeEc(sslerr::errc::invalid_context) };
	}
	...
	if (output.size() < required) {
		return { {}, sslerr::MakeEc(sslerr::errc::buffer_too_small) };
	}
...
std::tuple<gsl::span<uint8_t>, misc::ErrorCode> AES::Decrypt(gsl::span<const uint8_t> input, gsl::span<uint8_t> output) {
	if (ctx_.Ctx() == nullptr) {
		return { {}, sslerr::MakeEc(sslerr::errc::invalid_context) };
	}
